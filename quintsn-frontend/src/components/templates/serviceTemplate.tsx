...
export default function ServiceTemplate({ data }: { data: any }) {
  ...
  return (
    <>
      ...
      <section className="px-4 py-24 min-h-screen relative">
        <head>
        ...
        <section
          ref={ref}
          className="relative h-[400px] sm:h-[500px] md:h-[600px] flex items-center justify-center overflow-hidden mb-24"
        >
          {/* Scroll-based animation */}
          <motion.div
            className="absolute inset-0 overflow-hidden bg-cover bg-center"
            style={{ backgroundImage: `url(${data.heroImage})`, y }}
          >
            {/* Floating shapes */}
            <motion.div
          ...
          >
            ...
            <p className="text-base sm:text-lg md:text-2xl text-white max-w-3xl mx-auto">
        ...
        {data.sections.map((section: any, i: number) => (
          <div
            key={i}
            className={`container mx-auto my-24 grid md:grid-cols-2 gap-12 items-center ${
          ...
          >
            ...
            >
              <h2 className="text-3xl sm:text-4xl md:text-4xl font-bold mb-6">{section.title}</h2>
              {Array.isArray(section.text)
                ? section.text.map((p: string, idx: number) => (
                    <p key={idx} className="text-base sm:text-lg md:text-lg leading-relaxed mb-4">{p}</p>
                  ))
                : <p className="text-base sm:text-lg md:text-lg leading-relaxed">{section.text}</p>}
            </motion.div>
